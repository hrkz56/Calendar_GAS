 ...
 * }
 */
function parseSchedule(text, selectedYear, selectedMonth) {
  const normalized = normalizeInput_(text);
  const lines = normalized.split('\n');
  const baseYear = resolveBaseYear_(selectedYear);
  const baseMonth = resolveBaseMonth_(selectedMonth);
  const events = [];
  const errors = [];
  const warnings = [];
  ...
  lines.forEach(function(rawLine, index) {
    ...
    const parsed = parseScheduleLine_(line, currentCategory, baseYear, baseMonth, lineNo);
...
function resolveBaseMonth_(selectedMonth) {
  if (selectedMonth === undefined || selectedMonth === null || String(selectedMonth).trim() === '') return null;
...
function parseScheduleLine_(line, currentCategory, baseYear, baseMonth, lineNo) {
...
function extractDate_(line, baseYear, baseMonth) {
...
function detectCategory_(line, currentCategory) {
  ...
  if (/資産負債入力/.test(line) || currentCategory === '終日') return '終日';
...
function buildTitle_(line, category, dateInfo, timeInfo, explicitAllDay) {
  ...
  if (category === '有給') {
    if (/午後休/.test(title)) return '午後休';
    if (/2時間休/.test(title)) return '2時間休';
    return '有給休暇';
  }
  if (category === '終日') return stripCategoryPrefix_(title) || '終日';
